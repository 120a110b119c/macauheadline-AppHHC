<template>
	<view class="page" :style="{height: (swiperheight)  + 'px',overflow:'hidden' }">
		...
		<view class="list">
			...
			<view class="loginNow" @click="registerEnter()">立即登錄</view>
	...
	export default {
		...
		methods:{
			registerEnter(){
				...
				let phone = region+this.userInfo.account;
				this.$Httpps
					.sttp({
						"action": 'newlogin',
						"userTel":phone,
			...
			initDate(){
				this.$set(this.userInfo,'account',uni.getStorageSync('userPhoneNumber'))
				let areaCode = uni.getStorageSync('userAreaCode')
				let codeValue = ''
				if(areaCode.indexOf('86')){
					codeValue = '86'
				} else if(areaCode.indexOf('852')){
					codeValue = '852'
				}else if(areaCode.indexOf('853')){
					codeValue = '853'
				}
				this.chooesRange = "+"+codeValue;
				this.areaNumbers = '00'+codeValue
			},
			//跳转注册界面
			toNewUser() {
				uni.navigateTo({
			...
			getRegPhoneCode() {
				if (this.clickGetCode !== "獲取驗證碼") {
					...
					let region = this.areaNumbers;
					let number = this.userInfo.account;
