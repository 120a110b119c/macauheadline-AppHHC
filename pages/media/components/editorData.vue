...
<template>
	<view class="page">
		<!-- 编辑中 -->
		<view class="boxWrap">
			<view v-for="(item,index) in editorData" :key="index" class="dataList">
				<view class="dataContent">
					...
					<view class="boxContent">
						...
						<view class="btns">
							...
							<u-button class="btn" shape="circle" size="mini" type="error" :ripple="true" @click="showModal(item.DatId)">提交審核</u-button>
						</view>
...
<script>
	export default{
		...
		methods:{
			goEditDat(index){
				let datId = this.editorData[index].DatId;
				uni.navigateTo({
			...
			showModal(datId){
				this.$emit("changeStatus",3,2,this.changeTitle,datId)
			},
		}
	}
</script>
